...
Cordova 6.2 以降向けのプロジェクトで、このプラグインを使用する場合、次の機能が追加されます。
...
4.  プラグインの設定に必要なパラメーターを入力します。入力後、 {{<guilabel name="OK">}}
    ...
    `TARGETS` | 文字列 | [ 任意 ] 暗号化対象のファイルが格納されているフォルダーを指定します。<ul><li>`www` フォルダー ( デフォルト ) 。</li><li>複数のフォルダーを設定する場合は、`,` ( カンマ ) で区切ります。<br />例：TARGETS＝www/data,www/js</li></ul>
